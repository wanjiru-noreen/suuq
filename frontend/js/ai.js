  ...
  if (runBtn && resultsCard && outputDiv) {
    runBtn.addEventListener("click", async () => {
      ...
      outputDiv.textContent =
        "Running diagnostic pattern analysis on operational metrics...";

      try {
        const response = await api.post("/ai/analyze", {});
        outputDiv.textContent =
          response.analysis ||
          "Analysis complete. No operational anomalies detected.";
      } catch (err) {
        outputDiv.textContent =
          err.message || "Failed to complete AI diagnostic analysis.";
      } finally {
