...
async function loadReportData() {
  try {
    ...
    document.getElementById("rep-products").textContent =
      data.totalProducts ?? 0;
    document.getElementById("rep-stock").textContent =
      data.totalStockItems ?? 0;
    document.getElementById("rep-debtors").textContent =
      `KSh ${(data.totalDebtorsOutstanding ?? 0).toLocaleString()}`;
    document.getElementById("rep-creditors").textContent =
      `KSh ${(data.totalCreditorsOutstanding ?? 0).toLocaleString()}`;

    document.getElementById("health-in").textContent =
      data.stockBreakdown?.inStock ?? 0;
    document.getElementById("health-low").textContent =
      data.stockBreakdown?.low ?? 0;
    document.getElementById("health-critical").textContent =
