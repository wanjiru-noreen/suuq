...
async function loadDashboardData() {
  try {
    ...
    if (data.userName) {
      document.getElementById('user-greeting').textContent = `Good morning, ${data.userName} 👋`;
    }
    ...
    document.getElementById('stock-in').textContent = data.stockBreakdown?.inStock ?? 0;
    document.getElementById('stock-low').textContent = data.stockBreakdown?.low ?? 0;
    document.getElementById('stock-critical').textContent = data.stockBreakdown?.critical ?? 0;
    document.getElementById('stock-out').textContent = data.stockBreakdown?.out ?? 0;

    document.getElementById('stat-debtors').textContent = `KSh ${(data.totalDebtorsOutstanding ?? 0).toLocaleString()}`;
    document.getElementById('stat-creditors').textContent = `KSh ${(data.totalCreditorsOutstanding ?? 0).toLocaleString()}`;
